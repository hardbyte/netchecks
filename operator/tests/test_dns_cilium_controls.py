...
def test_k8s_version_with_installed_operator(
    ...
    subprocess.run(
        f"kubectl apply -n {k8s_namespace} -f {dns_restrictions_dir}/dns-netpol.yaml",
        shell=True,
        check=True,
    )
    time.sleep(5)
    ...
    subprocess.run(
        f"kubectl apply -n {k8s_namespace} -f {dns_restrictions_dir}",
        shell=True,
        check=True,
    )
    ...
    for i in range(10):
        jobs_response = subprocess.run(
            f"kubectl get jobs -n {k8s_namespace} -l app.kubernetes.io/instance=dns-restrictions-should-work",
            shell=True,
            check=True,
            capture_output=True,
        )
    ...
    subprocess.run(
        f"kubectl wait Job -l app.kubernetes.io/instance=dns-restrictions-should-work -n {k8s_namespace} --for condition=complete --timeout=120s",
        shell=True,
        check=True,
    )
    ...
    policy_report_results_response = subprocess.run(
        f"""kubectl get policyreport/dns-restrictions-should-work -n {k8s_namespace} -o {results_filter}""",
        shell=True,
        check=True,
        capture_output=True,
    )
