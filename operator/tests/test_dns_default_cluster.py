...
def test_k8s_version_with_installed_operator(netchecks, k8s_namespace, test_file_path):
    ...
    subprocess.run(f"kubectl wait Job/{name} -n {k8s_namespace} --for condition=complete --timeout=120s", shell=True, check=True)
    ...
    policy_report_summary = subprocess.run(f"""kubectl get policyreport/{name} -n {k8s_namespace} -o {summary_filter}""", shell=True, check=True, capture_output=True)
    ...
    policy_report_results_response = subprocess.run(f"""kubectl get policyreport/{name} -n {k8s_namespace} -o {results_filter}""", shell=True, check=True, capture_output=True)
    ...
    subprocess.run(f"kubectl delete -n {k8s_namespace} -f {manifest}", shell=True, check=True)
